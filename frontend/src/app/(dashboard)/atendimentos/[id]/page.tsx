  ...
  if (loadAtendimento) {
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center p-6">
        ...
        <p className="text-sm text-[#87938F]">Carregando atendimento...</p>
      </div>
    );
  }
  ...
  if (isError || !atendimento) {
    return (
      <div className="p-6 max-w-3xl mx-auto text-center py-20">
        <h2 className="text-xl font-bold text-[#F0EADD] mb-2">Atendimento não encontrado</h2>
        <p className="text-sm text-[#87938F] mb-6">O atendimento selecionado não existe ou pertence a outro estúdio.</p>
        <button
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-[#F0EADD] mt-2">Detalhes do Atendimento</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-[#0B171C] border border-[#243337] rounded-[18px] p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <span className="text-xs text-[#87938F] block mb-0.5">Parte do corpo</span>
                <span className="text-[#F0EADD] font-medium">{atendimento.parte_corpo || "—"}</span>
              </div>
              <div>
                <span className="text-xs text-[#87938F] block mb-0.5">Tamanho aproximado</span>
                <span className="text-[#F0EADD] font-medium">{atendimento.tamanho_cm || "—"}</span>
              </div>
              <div>
                <span className="text-xs text-[#87938F] block mb-0.5">Data da Sessão</span>
                <span className="text-[#F0EADD] font-medium flex items-center gap-1.5">
            ...
            {atendimento.descricao && (
              <div className="border-t border-[#243337] pt-4 mt-2">
                <span className="text-xs text-[#87938F] block mb-1.5">Descrição detalhada</span>
                <p className="text-sm text-[#F0EADD] leading-relaxed whitespace-pre-wrap bg-[#050B12] border border-[#243337]/50 rounded-[12px] p-3">
                  {atendimento.descricao}
                </p>
              </div>
            )}
          ...
          <div className="bg-[#0B171C] border border-[#243337] rounded-[18px] p-5 space-y-4">
            <h3 className="text-xs font-bold text-[#87938F] uppercase tracking-wider flex items-center gap-1.5">
              <ImageIcon size={14} className="text-[#2F9285]" /> Fotos de Referência
            </h3>

            {carregandoImagens ? (
              <div className="flex items-center justify-center py-10 text-sm text-[#87938F] gap-2">
                ...
                {imagens.map((filename) => {
                  ...
                  return (
                    <button
                      ...
                      className="aspect-square bg-[#050B12] border border-[#243337] rounded-[12px] overflow-hidden hover:border-[#2F9285]/60 transition-colors group relative"
                    >
                      <img
                        src={imgUrl}
                        alt="Referência"
              ...
              <div className="text-center py-10 border border-dashed border-[#243337] rounded-[14px]">
                <p className="text-sm text-[#87938F]">Nenhuma foto de referência anexada.</p>
              </div>
            )}
          </div>
        ...
        <div className="md:col-span-1 space-y-6">
          ...
          <div className="bg-[#0B171C] border border-[#243337] rounded-[18px] p-5 space-y-4">
            <h3 className="text-xs font-bold text-[#87938F] uppercase tracking-wider">Contato do Cliente</h3>
            {contato ? (
          ...
          <div className="bg-[#0B171C] border border-[#243337] rounded-[18px] p-5">
            <form onSubmit={handleSave} className="space-y-4">
              <h3 className="text-xs font-bold text-[#87938F] uppercase tracking-wider mb-2">Avaliação & Status</h3>

              {/* Status */}
              <div>
                <label className="block text-xs font-medium text-[#87938F] mb-1.5">Status Operacional</label>
                <select
                  value={statusOp}
              ...
              {/* Notas Privadas */}
              <div>
                <label className="block text-xs font-medium text-[#87938F] mb-1.5">Notas Privadas</label>
                <textarea
                  value={notasPrivadas}
                  onChange={(e) => setNotasPrivadas(e.target.value)}
                  placeholder="Observações internas..."
                  rows={4}
                  className="w-full px-3 py-2 rounded-[12px] bg-[#050B12] border border-[#243337] text-[#F0EADD] text-sm focus:border-[#2F9285]/60 outline-none transition-colors resize-none"
                />
              </div>

              {erro && (
                <div className="p-3 rounded-[10px] bg-[#E35D5B]/10 border border-[#E35D5B]/30 text-xs text-[#E35D5B]">{erro}</div>
              )}
      ...
      {imagemExpandida && (
        ...
        >
          <button
            type="button"
            className="absolute top-4 right-4 p-2 rounded-full bg-black/40 text-[#87938F] hover:text-[#F0EADD] border border-[#243337]"
            onClick={() => setImagemExpandida(null)}
          >
            <X size={24} />
          </button>
